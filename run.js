...
dclient.on('message', message => {
    ...
    if (message.content.startsWith(prefix + 'bf1')) {
        ...
        if (isSubscriber(message.author.id)) {
            try {
                ...
                var bf1basicstats = { url: "https://battlefieldtracker.com/bf1/api/Stats/BasicStats?platform=" + plat + "&displayName=" + playername + "&game=tunguska", headers: { 'trn-api-key': config.bf1trnkey } };

                var bf1detailedstats = { url: "https://battlefieldtracker.com/bf1/api/Stats/DetailedStats?platform=" + plat + "&displayName=" + playername + "&game=tunguska", headers: { 'trn-api-key': config.bf1trnkey } };
    ...
    if (message.content.startsWith(prefix + 'fnl')) {
        ...
        if (isSubscriber(message.author.id)) {
            try {
                ...
                var fortnitestatsapi = { url: "https://api.fortnitetracker.com/v1/profile/" + plat + "/" + playername, headers: { 'trn-api-key': config.fnltrnkey } };
                ...
                request(fortnitestatsapi, function(err, res, body) {
                    ...
                    if (!err && typeof info.error == 'undefined') {
                        ...
                        embed.addField("Name: ", playername, true)
                        embed.addField("Platform: ", plat, true)
                        embed.addField("Score: ", info.stats.p2.score.value, true)
                        embed.addField("K/D: ", info.stats.p2.kd.value, true)
                        embed.addField("KILLS: ", info.stats.p2.kills.value, true)
                        embed.addField("Play Time: ", info.stats.p2.minutesPlayed.displayValue, true)
                        message.channel.send({ embed });
                    } else {
                        let embed = new Discord.RichEmbed();
                        embed.setColor(0x9900FF)
    ...
    if (message.content.startsWith(prefix + 'Subs')) {
        if (isSubscriber(message.author.id)) {
            ...
            request("https://www.googleapis.com/youtube/v3/search?part=snippet&q=" + id + "&key=" + youtube_api_key, function(err, resp, body) {
                try {
                    var parsed = JSON.parse(body);
                    if (parsed.pageInfo.resultsPerPage != 0) {
                        for (var i = 0; i < parsed.items.length; i++) {
                            if (parsed.items[i].id.channelId) {
                                request("https://www.googleapis.com/youtube/v3/channels?part=statistics&id=" + parsed.items[i].id.channelId + "&key=" + youtube_api_key, function(err, resp, body) {
